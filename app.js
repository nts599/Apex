...
function initBlockerButtons() {
  ...
  const blockAllBtn = document.querySelector(".pill-btn");
  ...
  if (blockAllBtn) {
    blockAllBtn.addEventListener("click", () => {
      ...
      const shouldTurnOn = [...toggles].some(
        (toggle) => !toggle.classList.contains("on")
      );

      toggles.forEach((toggle) => {
        toggle.classList.toggle("on", shouldTurnOn);
      });
    });
  }
  ...
  const modeButtons = document.querySelectorAll(".mode-row");
...
function initSessionButtons() {
  ...
  // SELECT SUBJECT
  const subjectButton = document.querySelector(".select-field");

  if (subjectButton) {
    subjectButton.addEventListener("click", () => {
      alert("Select Subject clicked");
    });
  }
  ...
  const beginButton = document.querySelector(".btn-green");
